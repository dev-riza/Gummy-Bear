...
- Frontend (Vercel): _добавить после деплоя_
- Backend (Railway): _добавить после деплоя_
- GitHub: _добавить ссылку на публичный репозиторий_
...
Рекомендуется: frontend — Vercel, backend — Railway.

1. Запушить репозиторий на GitHub.
2. Backend в Railway: указать `backend/` как корень сервиса, команда
   запуска `uvicorn app.main:app --host 0.0.0.0 --port $PORT`,
   переменная `CORS_ORIGINS=https://<ваш-домен>.vercel.app`.
